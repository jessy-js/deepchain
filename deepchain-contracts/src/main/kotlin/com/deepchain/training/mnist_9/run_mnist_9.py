...
from hipsternet.constant import worker_num9
n_iter = 1500                                                                                                                                                                                                                                                                                                                                                                                             
#alpha = 1e-2 adam
alpha =5e-1
mb_size = 64
...
if __name__ == '__main__':
    ...
    val_set = [[] for i in range(worker_num9)]
    data = Split_data(mnist,worker_num9)
    X_train,y_train,\
    sample_train_x,sample_train_y,\
    train_x,train_y,\
    test_x,test_y,\
    vali_x,vali_y= data.split_data()
    for i in range(worker_num9):
    ...
    for k in range(n_experiment):
        ...
        net = solver_fun(
            net, X_train, y_train, worker_num=worker_num9,val_set=val1_set, mb_size=mb_size, alpha=alpha,
            n_iter=n_iter, print_after=print_after
        )
        y_pred = []
        accs=[]
