...
class ConvNet(NeuralNet):
    ...
    def forward(self, X, train=False):
        gamma1 ,gamma_poo11,gamma2,gamma_pool2,gamma3 = self.model['gamma1'],self.model['gamma_poo11'],self.model['gamma2'],self.model['gamma_pool2'],self.model['gamma3']
        beta1, beta_pool1,beta2,beta_pool2,beta3 = self.model['beta1'],self.model['beta_pool1'],self.model['beta2'], self.model['beta_pool2'],self.model['beta3']

        u1,u2,u3 ,upool1,upool2= None,None,None,None,None
        bn1_cache, pool1_cache,bn2_cache,pool2_cache ,bn3_cache= None, None,None,None,None
        ...
        #if train:
        #    h1 ,u1 = l.dropout_forward(h1,self.p_dropout)
        #print('h1 shape {}, u1 shape {}'.format(h1.shape,u1.shape))

        # Pool-1
        hpool1, hpool_cache1 = l.maxpool_forward(h1)
        #print('hpool1 shape',hpool1.shape)
        #pool1_cache = (self.bn_caches['bn_pool1_mean'],self.bn_caches['bn_pool1_var'])
        #hpool1,pool1_cache,run_mean,run_var = l.conv_bn_forward(hpool1,gamma_poo11,beta_pool1,pool1_cache,train= train)
        h2 = hpool1.ravel().reshape(X.shape[0], -1)

        #self.bn_caches['bn_pool1_mean'],self.bn_caches['bn_pool1_var'] = run_mean,run_var
        
        #if train:
        #   hpool1,upool1 = l.dropout_forward(hpool1,self.p_dropout)
        #print('hpool1 shape {} ,upool1 shape {}'.format(hpool1.shape,upool1.shape))
        #conv2 
        #bn2_cache = (self.bn_caches['bn2_mean'],self.bn_caches['bn2_var'])
        ...
        #pool-2
        hpool2,hpool_cache2 = l.maxpool_forward(h2)
        #pool2_cache = (self.bn_caches['bn_pool2_mean'],self.bn_caches['bn_pool2_var'])
        #hpool2,pool2_cache,run_mean,run_var = l.conv_bn_forward(hpool2,gamma_pool2,beta_pool2,pool2_cache,train = train)
        #print('hpool2 shape',hpool2.shape)
        
        hpool2_new = hpool2.ravel().reshape(hpool1.shape[0],-1)
        #print('hpool2 new shape',hpool2_new.shape)
        #if  train:
        #    hpool2_new,upool2 = l.dropout_forward(hpool2_new,self.p_dropout)

        #print('hpool2 shape',hpool2.shape)
        #print('upool2 shape ',upool2.shape)
        
        # FC-7
        h3, h3_cache = l.fc_forward(hpool2_new, self.model['W3'], self.model['b3'])
        #bn3_cache  = (self.bn_caches['bn3_mean'],self.bn_caches['bn3_var'])
        #h3, bn3_cache, run_mean, run_var = l.bn_forward(h3, gamma3, beta3, bn3_cache, train=train)
        #h3, nl_cache3 = l.relu_forward(h3)
        h3, nl_cache3 = l.relu_forward(h3)
        ...
        #if train:
        #    h3,u3 = l.dropout_forward(h3,self.p_dropout)
        #print('h3 shape {} ,u3 shape {}'.format(h3.shape,u3.shape))
        # Softmax
        # forth layer
        ...
        return score, (X, h1_cache,h2_cache, h3_cache, score_cache, hpool_cache1, hpool1, hpool_cache2,hpool2,nl_cache1, nl_cache2,nl_cache3)
            #bn1_cache,pool1_cache,bn2_cache,pool2_cache,bn3_cache)
    
    def backward(self, y_pred, y_train, cache):
        
        #X, h1_cache, h3_cache, score_cache, hpool_cache, hpool, \
        #nl_cache1,nl_cache3,\
        #u1,u2,u3,\
        #bn1_cache,pool_cache,bn3_cache = cache
        
        X, h1_cache, h2_cache,h3_cache, score_cache, hpool_cache1, hpool1, hpool_cache2,hpool2,nl_cache1, nl_cache2,nl_cache3= cache
       
        # Output layer
        grad_y = self.dloss_funs[self.loss](y_pred, y_train)
        
        #last layer
        dh3, dW4, db4 = l.fc_backward(grad_y, score_cache)
        
        #FC
        dh3 = self.backward_nonlin(dh3, nl_cache3) 
        #dh3,dgamma3,dbeta3= l.bn_backward(dh3,bn3_cache)  
        dpool2_new, dW3, db3 = l.fc_backward(dh3, h3_cache)

        #reshape
        #dpool2 = l.dropout_backward(dpool2_new,upool2)
        dpool2 = dpool2_new.ravel().reshape(hpool2.shape)

        #Pool-2
        #dpool2,dpool2_gamma,dpool2_beta = l.conv_bn_backward(dpool2,pool2_cache)
        dpool2 = l.maxpool_backward(dpool2, hpool_cache2)
      
        #conv -2
        #dh2 = l.dropout_backward(dpool2,u2)
        dh2 = self.backward_nonlin(dpool2,nl_cache2)
        #dW3+=reg.dl2_reg(self.model['W3'],self.lam)
        #dh2,dgamma2,dbeta2 = l.conv_bn_backward(dh2,bn2_cache)
        dh2 ,dW2,db2 = l.conv_backward(dh2,h2_cache)

        #Pool-1
        #dpool1,dpool1_gamma,dpool1_beta = l.conv_bn_backward(dh2,pool1_cache)
        #dpool1 = l.dropout_backward(dh2,upool1)
        dpool1 = l.maxpool_backward(dh2, hpool_cache1)    
        # Conv-1
      
        #dh1 = l.dropout_backward(dpool1,u1)
        dh1 = self.backward_nonlin(dpool1, nl_cache1)
        #dh1,dgamma1,dbeta1 = l.conv_bn_backward(dh1,bn1_cache)
        dh1, dW1, db1 = l.conv_backward(dh1, h1_cache)
        ...
        grad = dict(
            W1=dW1,W2 = dW2, W3=dW3, W4=dW4, b1=db1, b2 =db2,b3=db3, b4=db4,
            #gamma1 = dgamma1,gamma_poo11 = dpool1_gamma,gamma2 = dgamma2,gamma_pool2 = dpool2_gamma,gamma3 = dgamma3,
            #beta1 = dbeta1,beta_pool1 = dpool1_beta,beta2 =dbeta2,beta_pool2 = dpool2_beta,beta3 = dbeta3
        )
    ...
    def _init_model(self, D, C, H):
        self.model = dict(
            ...
            W3=np.random.randn(2*D * 7 * 7, H) / np.sqrt(2*D * 6 * 6 / 2.),
            W4=np.random.randn(H, C) / np.sqrt(H / 2.),
            b1=np.zeros((D, 1)),
            b2= np.zeros((2*D,1)),
            b3=np.zeros((1, H)),
            b4=np.zeros((1, C)),
            gamma1 = np.ones((1,D)),
            gamma_poo11 = np.ones((1,D)),
            gamma2 = np.ones((1,2*D)),
            gamma_pool2 = np.ones((1,2*D)),
            gamma3 = np.ones((1,H)),
            beta1  = np.zeros((1,D)),
            beta_pool1  = np.zeros((1,D)),
            beta2 = np.zeros((1,2*D)),
            beta_pool2 = np.zeros((1,2*D)),
            beta3  = np.zeros((1,H))
        )
        
        self.bn_caches=dict(
            bn1_mean = np.zeros((1,D)),
            bn_pool1_mean = np.zeros((1,D)),
            bn2_mean = np.zeros((1,2*D)),
            bn_pool2_mean = np.zeros((1,2*D)),
            bn3_mean = np.zeros((1,H)),
            bn1_var  = np.zeros((1,D)),
            bn_pool1_var = np.zeros((1,D)),
            bn2_var  = np.zeros((1,2*D)),
            bn_pool2_var = np.zeros((1,2*D)),
            bn3_var  = np.zeros((1,H))
            )
